...
export default function IpoFeedbackFormPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const application = (dashboardRes.data?.applications ?? []).find((item) => item.id === params.applicationId);
      if (!application) {
  ...
  async function submit() {
    if (!form || !params.applicationId) return;
    setSubmitting(true);
    setError(null);
    try {
      await fetchWithSession(`/api/industry/applications/${params.applicationId}/feedback`, {
        method: 'POST',
