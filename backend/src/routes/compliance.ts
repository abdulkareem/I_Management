import { buildReportTemplate, getPaymentRule, validateEvaluationMarks, validateInternshipCompliance } from '@prism/compliance';
...
const internshipDraftSchema = z.object({
  ...
  partnerYearsOfExperience: z.number().int().nonnegative().nullable().optional(),
...
export const complianceRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/compliance/payment-rule/:source', async (request) => {
    const params = z.object({ source: z.enum(['INTERNAL', 'EXTERNAL']) }).parse(request.params);
    return getPaymentRule(params.source);
  });

  app.post('/compliance/validate-evaluation', async (request) => {
    const payload = z.object({ ccaMarks: z.number(), eseMarks: z.number() }).parse(request.body);
    return {
